class StaffShiftsCreator
    ...
    def self.call(start_term, end_term, staff)
        self.new(start_term, end_term, staff).call
    end
    ...
    def start_time(date)
        Time.parse("#{date}").change(hour: holiday(date).work_start_at.hour, min: holiday(date).work_start_at.min)
    end

    def end_time(date)
        Time.parse("#{date}").change(hour: holiday(date).work_end_at.hour, min: holiday(date).work_end_at.min)
    end
end