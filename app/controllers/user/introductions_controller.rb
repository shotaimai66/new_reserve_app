class User::IntroductionsController < User::Base
  ...
  def params_user_calendar
    params.require(:user).permit(:name, calendars_attributes: %i[end_date display_week_term calendar_name phone public_uid display_interval_time end_time start_time display_time address])
  end
  ...
  def has_calendar?
    redirect_to introductions_new_staff_url if current_user.calendars.any?
  end
